...
def getPolynomialCoefficients(data4PolynomialFitting, joints,
    ...
    if removeBadHipFlexionEntries:
        ...
        # In some cases, the hip flexion moment arms are bad. 
        # We want to remove those entries for the polynomial fitting.
        ...
        idx_glmax1 = muscles.index('glut_max1_' + side)
        idx_glmax2 = muscles.index('glut_max2_' + side)
        idx_glmax3 = muscles.index('glut_max3_' + side)  
        momentArms_hip_flexion_glmax1 = momentArms[:,idx_glmax1, idx_hip_flexion]    
        idx_bad_hip_flexion_glmax1 = np.where(momentArms_hip_flexion_glmax1 >= 0)[0]
        ...
        idx_bad_hip_flexion_iliacus = np.where(momentArms_hip_flexion_iliacus <= 0.003)[0]
        idx_psoas = muscles.index('psoas_' + side) 
        momentArms_hip_flexion_psoas = momentArms[:,idx_psoas, idx_hip_flexion]    
        idx_bad_hip_flexion_psoas = np.where(momentArms_hip_flexion_psoas <= 0.0020)[0] 
        ...
        idx_bad_hip_add_iliacus = np.where(momentArms_hip_add_iliacus <= 0)[0]  
        # idx_bad_hip_add = np.concatenate((idx_bad_hip_add_iliacus))
        idx_bad_hip_add = idx_bad_hip_add_iliacus
        ...
        # idx_bad_hip_rot = np.concatenate((idx_bad_hip_rot_iliacus))
        idx_bad_hip_rot = idx_bad_hip_rot_iliacus
        
        # idx_bad_hip = idx_bad_hip_flexion
        ...
        idx_bad = idx_bad_hip #, idx_bad_ankle_edl))
